...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.groupBy(c => c).map(s => (s._1, s._2.size)).toList.sortWith(_._1 < _._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    occurrences.foldRight(List[Occurrences](Nil)) { (elem, acc) =>
      acc ++ (
        for {
          xs <- acc
          n <- 1 to elem._2
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    x.foldLeft(List[(Char, Int)]())((a : List[(Char, Int)], b : (Char, Int)) => {
      y.filter(_._1 == b._1 ) match {
        case List() => a :+ b
        case (h, b._2) :: xs => a
        case (h, occ) :: xs if occ < b._2 => a :+ (h, b._2 - occ)
      }
    })
  }
